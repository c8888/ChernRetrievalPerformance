...
meanCkAbs::usage =
    "meanCkAbs[ckTab_, q_, ckSupportMemberTable] takes a table of ck and returns Abs of them averaged over the same
    type of nodes. Used as an additional phase retrieval constraint. ckSupportMemberTable tells which node lies
    inside the support and which not."

trapezeSupport::usage =
    "trapezeSupport[trapezeRatio_, cellsRangeX_, cellsRangeY_, cellsSpaceX_, cellsSpaceY_, fullSpaceNodes_, dimx_,
    dimy_] creates a trapeziodal support and returns table {support, ckSupportMemberTable} where support is a
    (dimx*dimy) table with 1 inside support and 0 otherwise and ckSupportMemberTable
     is a table corresponding to table generated by wannierProject and its elements are 1 if the node is in the
     support and 0 otherwise."

findSigmaxyPeak::usage =
    "findSigmaxyPeak[measuredAbsSq_, dimBZx_, dimBZy_] finds the highest Bragg peak and returns its widths in XY
    directions by fitting gaussian."
...
meanCkAbs[ckTab_, q_, ckSupportMemberTable_] :=
    Normalize@Table[
      Total[Abs[(ckTab*ckSupportMemberTable)[[
          1 + i*Length[ckTab]/q ;; (i + 1)*Length[ckTab]/q]]]]/
          Total[ckSupportMemberTable[[
              1 + i*Length[ckTab]/q ;; (i + 1)*Length[ckTab]/q]]], {i, 0,
        q - 1}]

trapezeSupport[trapezeRatio_, cellsRangeX_, cellsRangeY_,
  cellsSpaceX_, cellsSpaceY_, fullSpaceNodes_, dimx_, dimy_] := Module[
  {
    trapezeCorners,
    trapeze,
    mf,
  ...
  trapezeCorners = {{cellsRangeX[[1]],
    cellsRangeY[[1]]}, {cellsRangeX[[1]],
    cellsRangeY[[2]]}, {cellsRangeX[[2]],
    cellsRangeY[[2]]}, {cellsRangeX[[2]], cellsRangeY[[1]]}};
  trapeze =
      Polygon[trapezeCorners + {{0,
        0}, {cellsSpaceX*(1 - trapezeRatio)/2,
        0}, {-cellsSpaceX*(1 - trapezeRatio)/2, 0}, {0, 0}}];
  mf = RegionMember[trapeze];
  support = Table[If[mf[{i, j}], 1, 0], {i, dimx}, {j, dimy}];
  ckSupportMemberTable = Map[If[mf[#[[1]]], 1, 0] &, fullSpaceNodes];
  Return[{support, ckSupportMemberTable}]
]

findSigmaxyPeak[measuredSq_, dimBZx_, dimBZy_] := Module[
  {
    posPeak =
        Flatten@Position[mirror2DSpace@measuredSq,
          Max[MaxDetect[measuredSq]*measuredSq]],
    peakData,
    peakDataXYZ,
    model,
    fit,
    ampl, x, ux, \[Sigma]x, y, uy, \[Sigma]y
  },
  peakData =
      mirror2DSpace[
        measuredSq][[posPeak[[1]] - Round[dimBZx/2] ;; posPeak[[1]] + Round[dimBZx/2],
          posPeak[[2]] - Round[dimBZy/2] ;; posPeak[[2]] + Round[dimBZy]]];
  peakDataXYZ =
      Flatten[MapIndexed[{#2[[1]], #2[[2]], #1} &, peakData, {2}], 1];
  model =
      ampl Exp[-(x - ux)^2/2/\[Sigma]x^2 - (y - uy)^2/2/\[Sigma]y^2];
  fit = NonlinearModelFit[peakDataXYZ,
    model, {{ampl, 1}, {ux, dimBZx/2}, {uy,
      dimBZy/2}, \[Sigma]x, \[Sigma]y}, {x, y}];
  Return[Abs@fit["ParameterTableEntries"][[4 ;; 5, 1]]]
]
